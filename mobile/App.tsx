...
import { riceLocalToISOString, riceToday, riceTodayLabel } from "./src/rice-time";
  ...
  const invite = async (person: Person) => {
    if (!isDemoMode) {
      const { error } = await supabase.from("event_members").upsert(
        { event_id: event.id, user_id: person.id, status: "invited" },
