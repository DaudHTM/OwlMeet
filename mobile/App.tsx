...
export default function App() {
  ...
  const refresh = useCallback(async (userId: string) => {
    const data = await loadNativeData(supabase, userId);
  ...
  const createEvent = async (draft: EventDraft) => {
    ...
    let inviteCode = id;
    if (!isDemoMode) {
      const startsAt = new Date(`${draft.date}T${to24Hour(draft.time)}`).toISOString();
...
function to24Hour(value: string) { const match = value.match(/^(\d{1,2}):(\d{2})\s*(AM|PM)$/i); if (!match) return "19:00:00"; let hour = Number(match[1]); if (match[3].toUpperCase() === "PM" && hour < 12) hour += 12; if (match[3].toUpperCase() === "AM" && hour === 12) hour = 0; return `${String(hour).padStart(2, "0")}:${match[2]}:00`; }
