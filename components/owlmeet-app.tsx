...
export function OwlMeetApp() {
  ...
  const upcoming = events.filter((event) => event.requested || event.invited || event.isOwner);
...
function EventCard({ event, featured, onSelect, onRequest }: { event: OwlEvent; featured?: boolean; onSelect: (event: OwlEvent) => void; onRequest: (id: string) => void }) {
  ...
  return (
    <article className={`event-card ${featured ? "featured" : ""}`} onClick={() => onSelect(event)}>
      ...
      <div className="card-footer"><div><AvatarStack people={event.attendees} /><span>{event.attendees.length} going · {spots} {spots === 1 ? "spot" : "spots"} left</span></div>
        {event.isOwner ? <button className="secondary" onClick={(e) => { e.stopPropagation(); onSelect(event); }}>{event.pending.length ? `${event.pending.length} requests` : "Manage"}</button> : <button className={event.requested ? "secondary success" : "primary compact"} disabled={event.requested} onClick={(e) => { e.stopPropagation(); onRequest(event.id); }}>{event.requested ? <><Check size={15} /> Requested</> : "Request to join"}</button>}
      </div>
    </article>
  );
}
